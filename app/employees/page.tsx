...
export default function EmployeesHREPage() {
  ...
  const ot_grandTotalHours = ot_hoursPerPerson * ot_selectedEmployeeIds.length
  ...
  }, [overtimeEntries, ot_filterEmployeeId, ot_filterMonth, ot_searchTerm])
  ...
  const handleGenerateCombinedReport = async (reportType: 'detailed' | 'merged' | 'summary') => {
    ...
    const employeesData: CombinedEmployeeData[] = selectedEmps.map(emp => {
      const empOvertime = overtimeEntries.filter(e => {
         const matchMonth = e.date?.startsWith(cr_month)
         const matchEmp = e.employeeIds?.includes(emp.id) || (e as any).employeeId === emp.id
         return matchMonth && matchEmp
      }).sort((a,b) => (a.date || '').localeCompare(b.date || ''))

      const empAbsences = absenceRecords.filter(r => {
         const matchMonth = r.date?.startsWith(cr_month)
         const matchEmp = r.employeeId === emp.id
         return matchMonth && matchEmp
      }).sort((a,b) => (a.date || '').localeCompare(b.date || ''))
  ...
  const at_categories = {
    absence: [
      { id: "unexcused", labelAr: "بدون عذر", labelEn: "Unexcused" },
      { id: "excused", labelAr: "بعذر", labelEn: "Excused" },
      { id: "sick", labelAr: "مرضي", labelEn: "Sick" },
    ],
    leave: [
      { id: "official", labelAr: "رسمية", labelEn: "Official" },
      { id: "eid", labelAr: "عيد", labelEn: "Eid" },
      { id: "national", labelAr: "يوم وطني", labelEn: "National Day" },
    ],
    official_event: [
      { id: "work_visit", labelAr: "زيارة عمل", labelEn: "Work Visit" },
      { id: "training", labelAr: "تدريب", labelEn: "Training" },
    ]
  }
        ...
        <Tabs defaultValue="overtime" className="w-full">
          <TabsList className="h-14 bg-white shadow-sm mb-6 border-2 w-full md:w-auto overflow-x-auto justify-start">
            ...
            <TabsTrigger value="reports" className="h-10 text-lg font-black px-8 data-[state=active]:bg-primary data-[state=active]:text-white">
              <Calculator className="h-5 w-5 ml-2" /> التقارير الشاملة
            </TabsTrigger>
          </TabsList>
                     ...
                     <div className="space-y-4">
                       ...
                       <div className="border-2 rounded-xl p-4 bg-slate-50/50 shadow-inner">
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                           {employees.map(emp => (
                             <div key={emp.id} className="flex items-center gap-3 hover:bg-white p-2 rounded-lg transition-all group border border-transparent hover:border-primary/20">
                               {editingEmployeeId === emp.id ? (
                                 ...
                                 <>
                                   <Checkbox id={`ot-emp-${emp.id}`} className="h-6 w-6 border-2" checked={ot_selectedEmployeeIds.includes(emp.id)} onCheckedChange={() => setOtSelectedEmployeeIds(prev => prev.includes(emp.id) ? prev.filter(i => i !== emp.id) : [...prev, emp.id])} />
               ...
               <Card className="shadow-xl border-slate-200 overflow-hidden">
                 <CardHeader className="bg-slate-900 text-white flex flex-row items-center justify-between py-6 px-8">
                   ...
                   <div className="flex gap-4">
                      <Button variant="outline" onClick={() => generateOvertimeReportPDF(ot_filteredEntries, ot_filterMonth, "تقرير", "detailed")} className="bg-white text-slate-900 font-black"><Printer className="ml-2 h-4 w-4"/> مفصل / Detailed</Button>
                      <Button variant="outline" onClick={() => generateOvertimeReportPDF(ot_filteredEntries, ot_filterMonth, "تقرير", "merged")} className="bg-white text-slate-900 font-black"><Printer className="ml-2 h-4 w-4"/> مدمج / Merged</Button>
                   </div>
                 </CardHeader>
                 <CardContent className="p-0">
                    <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 border-b">
                      <Input placeholder="بحث..." value={ot_searchTerm} onChange={e => setOtSearchTerm(e.target.value)} className="h-12" />
                      <Select value={ot_filterEmployeeId} onValueChange={setOtFilterEmployeeId}>
                        <SelectTrigger className="h-12"><SelectValue placeholder="الموظف"/></SelectTrigger>
                        <SelectContent>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                   <Card className="shadow-lg border-primary/10">
                     ...
                     <CardContent className="p-6 space-y-4">
                        <div className="flex justify-between p-4 bg-red-50 rounded-xl border border-red-100"><div className="font-black text-red-700">غياب</div><div className="text-3xl font-black text-red-900">{convertNumbersToEnglish(at_filteredRecords.filter(r => r.type==='absence').length)}</div></div>
                        <div className="flex justify-between p-4 bg-orange-50 rounded-xl border border-orange-100"><div className="font-black text-orange-700">إجازة</div><div className="text-3xl font-black text-orange-900">{convertNumbersToEnglish(at_filteredRecords.filter(r => r.type==='leave').length)}</div></div>
                   ...
                   <Card className="shadow-lg p-4 space-y-4">
                      ...
                      <Select value={at_filterEmployeeId} onValueChange={setAtFilterEmployeeId}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">الكل</SelectItem>{employees.map(e => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}</SelectContent></Select>
                      <Input type="month" value={at_filterMonth} onChange={e => setAtFilterMonth(e.target.value)} />
                      <Input placeholder="بحث..." value={at_searchTerm} onChange={e => setAtSearchTerm(e.target.value)} />
                   </Card>
                </div>
                ...
                <div className="lg:col-span-3">
                   <Card className="shadow-xl overflow-hidden border-slate-200">
                     ...
                     <CardContent className="p-0">
                       <table className="w-full text-right border-collapse">
                         <thead className="bg-slate-50 border-b"><tr><th className="p-4">الموظف</th><th className="p-4 text-center">التاريخ</th><th className="p-4 text-center">النوع</th><th className="p-4 text-center">الفئة</th><th className="p-4 text-center">إجراءات</th></tr></thead>
                         <tbody className="divide-y">
               ...
               <CardContent className="p-8 space-y-6 flex flex-col items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mt-4">
                    <Button 
                      ...
                      className="h-16 text-lg font-black shadow-xl bg-primary hover:bg-primary/90"
                      disabled={cr_selectedEmployeeIds.length === 0}
                    >
                      <Printer className="h-6 w-6 ml-2" /> تقرير مفصل
                    </Button>
                    <Button 
                      onClick={() => handleGenerateCombinedReport('merged')} 
                      className="h-16 text-lg font-black shadow-xl border-primary text-primary hover:bg-primary/10"
                      variant="outline"
                      disabled={cr_selectedEmployeeIds.length === 0}
                    >
                      <Printer className="h-6 w-6 ml-2" /> تقرير مدمج (خلاصة)
                    </Button>
                    <Button 
                      onClick={() => handleGenerateCombinedReport('summary')} 
                      className="h-16 text-lg font-black shadow-xl border-orange-500 text-orange-600 hover:bg-orange-50"
                      variant="outline"
                      disabled={cr_selectedEmployeeIds.length === 0}
                    >
                      <Printer className="h-6 w-6 ml-2" /> تقرير مختصر
                    </Button>
                  </div>
               </CardContent>
             </Card>
